...
const createNewWord = (): Partial<Item> => ({ TranslateVariants: [''] });
const newWord = ref<Partial<Item>>(createNewWord());
const saveNewWord = async (): Promise<void> => {
    ...
    if (!selectedLanguage.value || !user?.uid || isSavingWord.value) {
        return;
    }

    isSavingWord.value = true;
    try {
        const targetPack = selectedPack.value?.code || listPacs.value.length + 1;
        ...
        toast.add({
            severity: 'success',
            summary: `Слово добавлено в Пак ${targetPack}`,
            life: 3000,
        });
        newWord.value = createNewWord();
    } finally {
    ...
    <div class="min-h-screen bg-gradient-to-br from-slate-100 to-gray-200 p-4 md:p-6">
        <div class="max-w-3xl mx-auto">
            ...
            <div class="bg-white rounded-2xl shadow-xl p-6 md:p-8">
                <ApiLoader v-if="isLoadingLanguages" message="Загружаем языки и паки..." />
